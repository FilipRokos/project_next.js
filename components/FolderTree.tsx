...
function TreeNode({ folder, folders, files, currentId, depth, onSelect }: TreeNodeProps) {
    ...
    return (
        <div>
            <div
                className={`w-full flex items-center gap-1 py-1 rounded-md text-xs transition group ${
                    isActive
            ...
            {open && (
                <div>
                    ...
                    {childFiles.map((file) => (
                        <div
                            key={file.id}
                            className="flex items-center gap-1.5 py-1 text-xs text-slate-400 truncate"
                            style={{ paddingLeft: `${6 + (depth + 1) * 12}px` }}
                        >
                            <span className="opacity-0 w-2 shrink-0" />
                            <FaFileImage size={9} className="shrink-0 text-emerald-400" />
    ...
    return (
        <div className="space-y-0.5">
            {roots.length === 0 && rootFiles.length === 0 && (
                <p className="text-xs text-slate-400 px-2 py-1">Prázdné</p>
            )}
            ...
            {rootFiles.map((file) => (
                <div
                    key={file.id}
                    className="flex items-center gap-1.5 py-1 px-1.5 text-xs text-slate-400 truncate"
                >
                    <span className="opacity-0 w-2 shrink-0" />
                    <FaFileImage size={9} className="shrink-0 text-emerald-400" />
                    <span className="truncate">{file.fileName}</span>
